...
def wprint(*to_file)-> None:
    ...
    with open('gameplay.txt', 'a', encoding='utf-8') as f:
...
def miroh_world_act1(inventory: list) -> bool:  # расписан шаг 1
    ...
    if move == "1":
        ...
        if sprint():
            wprint("Вы проскочили и идете дальше")
            return True
        ...
        if selected_item == "фонарик":
            wprint("Вы ослепили робота и спокойно прошли")
            return True
...
def meeting() -> bool:
    ...
    if choice == "1":
        ...
        wprint("Вы тратите время, но помогаете людям.")
...
def miroh_world_act2(inventory: list) -> bool:
    ...
    while steps < 5:
        ...
        if steps == 3:
            result = meeting()
            wprint("После встречи с беженцами вы проходите оставшиеся перекрёстки")
    if road == "12321":
        return True
...
def use_tool(inventory: list[str], step: int) -> None:
    ...
    if use == "2":
        return None
    show_inventory(inventory)
    wprint("0 - Ничего (вернутся к ответам)")
    ...
    if selected_item == "фонарик":
        ...
        wprint(f"Дух разозлится от ответа {rude_answers[step - 1]}")
        return None
    elif selected_item == "компас":
        wprint(
        ...
        wprint(f"Дух ждет ответ {strong_answers[step - 1]}")
        return None
    elif selected_item == "амулет":
        wprint("Поздравляю, вы теряете амулет! а ведь это могло спасти вам жизнь..")
        inventory.remove("амулет")
        game["unnecessary_use_amulet_count"] += 1
        return None
...
def star_world_performance(inventory: list[str]) -> int:
    ...
    if ans == "4":
        ...
        selected_item = inventory[int(item) - 1]
        if selected_item == "фонарик":
            wprint("Фонарик подсвечивает что не стоит повторять чужой стиль")
        elif selected_item == "компас":
            wprint("Компас указывает что лучше выбрать между импровизацией и сальто")
        elif selected_item == "амулет":
            wprint("Поздравляю, вы теряете амулет! а ведь это могло спасти вам жизнь..")
            inventory.remove("амулет")
            game["unnecessary_use_amulet_count"] += 1
    ...
    if ans == "1":
        wprint("Выступление прошло энергично, зрители поддерживают.")
        return 1
    elif ans == "2":
        wprint("Копия выглядит бледно, публика чувствует фальшь.")
        return 2
    elif ans == "3":
        wprint("Вот это и есть стиль 5 STAR!")
        return 3
...
def star_world_gameplay(inventory: list[str]) -> bool:
    ...
    wprint("Участники зафиксировали твой выбор \n" "Ты выступаешь в стиле ", traits)
    ...
    if users_style == "1":
        wprint("Зал реагирует, но чувствуется излишняя агрессия.")
        stars_collected += 1

        game["rudeness_count"] += 1
    elif users_style == "2":
        wprint("Неуверенность снижает поддержку зала.")

        wprint("Из тишины зала издается..")
        get_achieve("HA-HA")
    elif users_style == "3":
        wprint("Зрители отвечают теплом, поддержка растёт.")
        stars_collected += 2
        game["kindness_count"] += 1
    if stars_collected >= 3:
        return True
    else:
        wprint("Вам не хватило уверенности. Символ не получен.")
        return False
...
def main():
    ...
    wprint("Благодаря выбранной сложности при себе вы имеете:")
    wprint(game["inventory"])
    ...
    n = 0
    wprint("Вы засыпаете и попадаете в SKZ‑Verse. Перед вами три мира:\n")
    while n < 3:
        ...
        if world_id == 1:
            ...
            if result:
                ...
                wprint("Вы выбрались из лабиринта! Вы получаете символ MIROH")
        elif world_id == 2:
            ...
            if result:
                ...
                wprint("Вы получаете символ 5-STAR. Вы показали себя настоящего.\n " \
                "Вы получаете символ 5-STAR")
        n += 1
    if len(game["symbols"]) == 3:
        ...
        wprint("Вы не смогли проснуться и остались в мире снов навсегда :(")
    ...
    if game["used_amulet"] >= 2:
        ...
        wprint("Вы скрытый 9 мембер Stray Kids, но лучше проходить все с первого раза.")
    if len(game["symbols"]) == 3 and game["unnecessary_use_amulet_count"] == 3:
        get_achieve("придумать название")
        wprint("придумать надпись")
